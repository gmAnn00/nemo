...
public class GroupMangerDAO {
	...
	//가입승인 대기 멤버 목록 메소드
	public List<UserVO> approveMemberShow(int grp_id) {
		List<UserVO> approveUserList = new ArrayList<>();
		try {
			conn = dataFactory.getConnection();
			
			String query = "SELECT user_tbl.* FROM user_tbl JOIN waitlist_tbl ON user_tbl.user_id = waitlist_tbl.user_id where grp_id=?";
			System.out.println(query);
	...
	public void mandateGroupManager(UserVO userVO, int grp_id) {
	 ...
	 public void exileGroupMember(UserVO userVO, int grp_id) {
		...
		try {
        	conn = dataFactory.getConnection();
        	String query = "delete from grpjoin_tbl where user_id=? and grp_id=?";			
        	System.out.println(query);
	 ...
	 public void rejectUser(UserVO userVO, int grp_id) {
		 String user_id=userVO.getUser_id();
			...
			try {
	        	conn = dataFactory.getConnection();
	        	String query = "DELETE FROM waitlist_tbl WHERE user_id = ? and grp_id=?";			
	        	System.out.println(query);
