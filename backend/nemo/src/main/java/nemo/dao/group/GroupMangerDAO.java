package com.nemo.web.group.dao;
...
public class GroupMangerDAO {
	...
	//현재 유저가 소모임장인지 체크(소모임장 정보 갖고오기) -> session id랑 비교
	public boolean grpMngCheck(String user_id) {
		boolean mngCheck=false;
		try {
        	conn = dataFactory.getConnection();
        	String query = "SELECT user_tbl.*, "
        			+ " CASE WHEN user_tbl.user_id = group_tbl.grp_mng THEN 'true' ELSE 'false' END AS RESULT"
        			+ " FROM user_tbl"
        			+ " LEFT JOIN group_tbl ON user_tbl.user_id = group_tbl.grp_mng"
	...
	//그룹보기 메소드
	public List<GroupVO> groupShow() {
		List<GroupVO> groupList = new ArrayList<>();
		try {
			conn = dataFactory.getConnection();
			
			String query = "select * from group_tbl where grp_id=1";	//일단 그룹정보를 갖고오는데!! 내가 가입한 여러 그룹중에서 갖고와야됨...!!!
			System.out.println(query);
			...
			while(rs.next()) {
				...
				GroupVO groupVO = new GroupVO(grp_id, grp_name, grp_mng, mem_no, grp_zipcode, grp_addr1, grp_addr2, create_date, grp_intro, app_st, main_name, sub_name, grp_mimg, grp_pimg);		
	...
	//멤버 목록 메소드
	public List<UserVO> userShow() {
		List<UserVO> userList = new ArrayList<>();
		try {
			conn = dataFactory.getConnection();
			
			String query = "SELECT user_tbl.* FROM user_tbl JOIN grpjoin_tbl ON user_tbl.user_id = grpjoin_tbl.user_id where grp_id=1";
			System.out.println(query);
	...
	public void mandateGroupManager(UserVO userVO) {
	 ...
	 public void rejectUser(UserVO userVO) {
			...
			try {
	        	conn = dataFactory.getConnection();
	        	String query = "DELETE FROM waitlist_tbl WHERE user_id=?";			
	        	System.out.println(query);
