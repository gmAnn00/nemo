...
public class MyScheduleDAO {
	...
	public List<Map> selectSchedule(String user_id, String currentYM) {
		...
		try {
			...
			String query = "SELECT g.grp_id, g.grp_name, g.grp_img, a.schedule, s.sche_title, s.sche_cont, s.location, substr(a.schedule, 7, 8) as day";
			query += " FROM attend_tbl a, group_tbl g, schedule_tbl s, user_tbl u";
			query += " WHERE u.user_id=? AND u.user_id=a.user_id AND g.grp_id=a.grp_id AND a.grp_id=s.grp_id AND a.schedule=s.schedule AND substr(a.schedule, 1, 5)=? order by a.schedule, g.grp_id";
			System.out.println(query);
			...
			while(rs.next()) {					
				...
				scheduleVO.setLocation(location);		
			 	...
			 	String scheduleDate = new SimpleDateFormat("yyyy년 MM월 dd일").format(schedule);	
			 	String scheduleMonth = new SimpleDateFormat("MM").format(schedule);
				String scheduleTime =  new SimpleDateFormat("HH시 mm분").format(schedule);
				...
				scheduleMap.put("grp_img", grp_img);
				scheduleMap.put("scheduleDate", scheduleDate);
				scheduleMap.put("scheduleMonth", scheduleMonth);
				scheduleMap.put("scheduleTime", scheduleTime);						
			...
			System.out.println("나의 이번달 전체 일정 조회 중 에러 ");
			e.printStackTrace();
		}	
		return scheduleList;
	}
